...
def load_regulation_data(regulation_name: str):
    """規制の構造化データを読み込む"""
    try:
        with open(f'{regulation_name.lower()}_data/{regulation_name.lower()}_structured.json', 'r', encoding='utf-8') as f:
            return json.load(f)
    except Exception as e:
        print(f"{regulation_name}データの読み込み中にエラー: {e}")
        return None
...
def check_existing_regulation(supabase, regulation_name: str):
    """既存の規制データをチェック"""
    try:
        response = supabase.table('regulations').select('*').eq('name', regulation_name).execute()
        return len(response.data) > 0
    except Exception as e:
        print(f"既存データのチェック中にエラー: {e}")
        return None

def delete_existing_regulation(supabase, regulation_name: str):
    """既存の規制データを削除"""
    try:
        # regulationsテーブルから対象の規制を検索
        response = supabase.table('regulations').select('id').eq('name', regulation_name).execute()
        ...
        print(f"既存の{regulation_name}データを削除しました")
        return True
    except Exception as e:
...
def upload_regulation_data(supabase, data, regulation_name: str, regulation_metadata: Dict[str, Any], chapter_article_mapping: Dict[str, str] = None):
    """
    規制データをアップロード
    Args:
        supabase: Supabaseクライアント
        data: 規制のJSON構造化データ
        regulation_name: 規制の名前
        regulation_metadata: 規制のメタデータ
        chapter_article_mapping: チャプターと条文の対応マップ（オプション）
    """
    try:
...
def main():
    try:
        ...
        # 既存のDMAデータをチェック
        print("\n既存のDMAデータをチェックしています...")
        has_existing = check_existing_regulation(supabase, 'DMA')
        if has_existing:
            ...
            print("\n既存のDMAデータを削除しています...")
            if not delete_existing_regulation(supabase, 'DMA'):
                print("既存データの削除に失敗しました。")
                return
        
        # データのアップロード
        print("\nDMAデータをアップロードしています...")
        if upload_regulation_data(supabase, data, 'DMA', {}):
